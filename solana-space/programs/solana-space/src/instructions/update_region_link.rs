...
pub struct UpdateRegionLinkArgs {
    pub new_link: String,
}

#[derive(Accounts)]
pub struct UpdateRegionLink<'info> {
    /// The NFT owner
...
pub fn update_region_link_handler(
    ...
    let image_uri = find_attr(&existing_attrs.attribute_list, "image_uri")?;

    // 5. Reconstruct attribute list with only link changed
    let attributes = vec![
        ...
        Attribute { key: "image_uri".to_string(), value: image_uri },
